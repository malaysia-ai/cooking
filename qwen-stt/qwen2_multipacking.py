...
class DataTrainingArguments:
    ...
    mosaic: bool = field(default=True, metadata={"help": "Enable mosaic mode"})
...
def main():
    ...
    for i in range(8192):
        extra.append(AddedToken(f'<|speech_index_{i}|>'))
    ...
    class DatasetFixed(torch.utils.data.Dataset):
        def __init__(self, local, mosaic = False):
            self.mosaic = mosaic
            if self.mosaic:
                ...
                for f in files:
                    folder = f.replace('.parquet', '')
                    df = pd.read_parquet(f).to_dict(orient = 'records')
                    for i in range(len(df)):
                        df[i]['token_filename'] = os.path.join(folder, f'{i}.json')
                    self.dataset.extend(df)
        ...
        def __getitem__(self, idx):
            ...
            if self.mosaic:
                ...
                try:
                    with open(data['token_filename']) as fopen:
                        token = json.load(fopen)
                except:
                    return
                    
                token = ''.join([f'<speech_{t}>' for t in token])
                prompt = f"<s>{data['ratio_bin']},{data['ma30_pitch_mean_bin']},{data['speaker']}: {data['normalized_generate_text']}<start_speech>{token}</s>"
    ...
    dataset = DatasetFixed(data_args.train_file, data_args.mosaic)
    ...
    def collator(batch):
        ...
        input_ids = [{'input_ids': b['input_ids'][0]} for b in batch]
        data = tokenizer.pad(input_ids, padding = 'max_length', max_length = sequence_length)
